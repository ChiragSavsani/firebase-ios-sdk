...
- (void)setUp {
  ...
  _testColUpdatedDocsValues = @[
    @{@"key1" : @"value1"}, @{@"key2" : @"value2", @"key2b" : @"value2b"}, @{@"key3b" : @"value3b"},
    @{@"key4" : @"value4"}
  ];
...
- (void)updateData {
  ...
  [[_testCol documentWithPath:@"doc2"] setData:@{@"key2b" : @"value2b"}
...
- (void)testGetDocumentWhileOnlineCacheOnly {
  ...
  FIRDocumentSnapshot *result =
      [self readDocumentForRef:_testDoc getOptions:[[FIRGetOptions alloc] initWithSource:FIRCache]];
  XCTAssertTrue(result.exists);
  XCTAssertTrue(result.metadata.fromCache);
  XCTAssertEqualObjects(result.data, _testDocInitialData);
}
...
- (void)testGetDocumentsWhileOnlineCacheOnly {
  ...
  FIRQuerySnapshot *result =
      [self readDocumentSetForRef:_testCol
                       getOptions:[[FIRGetOptions alloc] initWithSource:FIRCache]];
  XCTAssertTrue(result.metadata.fromCache);
  XCTAssertEqualObjects(FIRQuerySnapshotGetData(result), _testColInitialDocsValues);
}
...
- (void)testGetDocumentWhileOnlineServerOnly {
  ...
  // the initialData.
  FIRDocumentSnapshot *result =
      [self readDocumentForRef:_testDoc
                    getOptions:[[FIRGetOptions alloc] initWithSource:FIRServer]];
  XCTAssertTrue(result.exists);
  XCTAssertFalse(result.metadata.fromCache);
  XCTAssertEqualObjects(result.data, _testDocInitialData);
}
...
- (void)testGetDocumentWhileOfflineServerOnly {
  ...
  [_testDoc getDocumentWithOptions:[[FIRGetOptions alloc] initWithSource:FIRServer]
...
- (void)testGetDocumentsWhileOfflineServerOnly {
  ...
  [_testCol getDocumentsWithOptions:[[FIRGetOptions alloc] initWithSource:FIRServer]
...
- (void)testGetDocumentWhileOfflineWithDifferentGetOptions {
  ...
  // the listener to be fully initialized before continuing.
  ...
  result = [self readDocumentForRef:_testDoc
                         getOptions:[[FIRGetOptions alloc] initWithSource:FIRDefault]];
  XCTAssertTrue(result.exists);
  XCTAssertTrue(result.metadata.fromCache);
  XCTAssertEqualObjects(result.data, _testDocUpdatedData);
...
- (void)testGetDocumentsWhileOfflineWithDifferentGetOptions {
  ...
  result = [self readDocumentSetForRef:_testCol
                            getOptions:[[FIRGetOptions alloc] initWithSource:FIRDefault]];
  XCTAssertTrue(result.metadata.fromCache);
  XCTAssertEqualObjects(FIRQuerySnapshotGetData(result), _testColUpdatedDocsValues);
...
- (void)testGetNonExistingDocWhileOnlineCacheOnly {
  ...
  XCTestExpectation *getNonExistingDocCompletion =
      [self expectationWithDescription:@"getNonExistingDoc"];
  [_emptyDoc getDocumentWithOptions:[[FIRGetOptions alloc] initWithSource:FIRCache]
                         completion:^(FIRDocumentSnapshot *snapshot, NSError *error) {
                           XCTAssertNotNil(error);
                           XCTAssertEqualObjects(error.domain, FIRFirestoreErrorDomain);
                           XCTAssertEqual(error.code, FIRFirestoreErrorCodeUnavailable);
...
- (void)testGetNonExistingDocsWhileOnlineCacheOnly {
  ...
  FIRQuerySnapshot *snapshot =
      [self readDocumentSetForRef:_emptyCol
                       getOptions:[[FIRGetOptions alloc] initWithSource:FIRCache]];
  XCTAssertTrue(snapshot.metadata.fromCache);
}
...
- (void)testGetNonExistingDocWhileOnlineServerOnly {
  ...
  FIRDocumentSnapshot *snapshot =
      [self readDocumentForRef:_emptyDoc
                    getOptions:[[FIRGetOptions alloc] initWithSource:FIRServer]];
  XCTAssertFalse(snapshot.exists);
  XCTAssertFalse(snapshot.metadata.fromCache);
}
...
- (void)testGetNonExistingDocWhileOfflineServerOnly {
  ...
  XCTestExpectation *getNonExistingDocCompletion =
      [self expectationWithDescription:@"getNonExistingDoc"];
  [_emptyDoc getDocumentWithOptions:[[FIRGetOptions alloc] initWithSource:FIRServer]
                         completion:^(FIRDocumentSnapshot *snapshot, NSError *error) {
                           XCTAssertNotNil(error);
                           XCTAssertEqualObjects(error.domain, FIRFirestoreErrorDomain);
                           XCTAssertEqual(error.code, FIRFirestoreErrorCodeUnavailable);
...
- (void)testGetNonExistingDocsWhileOfflineServerOnly {
  ...
  // attempt to get docs and ensure they cannot be retreived
  XCTestExpectation *failedGetDocsCompletion = [self expectationWithDescription:@"failedGetDocs"];
  [_emptyCol getDocumentsWithOptions:[[FIRGetOptions alloc] initWithSource:FIRServer]
                          completion:^(FIRQuerySnapshot *snapshot, NSError *error) {
                            XCTAssertNotNil(error);
                            XCTAssertEqualObjects(error.domain, FIRFirestoreErrorDomain);
                            XCTAssertEqual(error.code, FIRFirestoreErrorCodeUnavailable);
